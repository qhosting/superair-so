...
import {
    ...
    FileUp, Download, Table, FileSpreadsheet
} from 'lucide-react';
...
const Inventory: React.FC = () => {
    ...
    const downloadTemplate = () => {
        const template = [
            {
                'Código (SKU)': 'AC-LG-12K',
                'Nombre': 'Aire Acondicionado LG 12000 BTU',
                'Categoría': 'Equipos AC',
                'Tipo (product/service)': 'product',
                'Unidad de Medida': 'Pza',
                'Costo': 4500.00,
                'Precio Venta': 7800.00,
                'Existencia Inicial': 10,
                'Stock Mínimo': 3
            },
            {
                'Código (SKU)': 'INST-BAS',
                'Nombre': 'Servicio de Instalación Básica',
                'Categoría': 'Mano de Obra',
                'Tipo (product/service)': 'service',
                'Unidad de Medida': 'Pza',
                'Costo': 800.00,
        ...
        XLSX.writeFile(wb, "Plantilla_Inventario_SuperAir.xlsx");
        ...
        reader.onload = (event) => {
            try {
                ...
                const parsed = json.map(row => ({
                    code: row['Código (SKU)'] || row['code'] || '',
                    name: row['Nombre'] || row['name'] || '',
                    category: row['Categoría'] || row['category'] || '',
                    type: row['Tipo (product/service)'] || row['type'] || 'product',
                    unit_of_measure: row['Unidad de Medida'] || row['unit_of_measure'] || 'Pza',
                    cost: parseFloat(row['Costo'] || row['cost']) || 0,
                    price: parseFloat(row['Precio Venta'] || row['price']) || 0,
                    stock: parseInt(row['Existencia Inicial'] || row['stock']) || 0,
                    min_stock: parseInt(row['Stock Mínimo'] || row['min_stock']) || 5
                }));

                if (parsed.length === 0) {
                    showToast("El archivo parece estar vacío o con formato incorrecto", "error");
                    return;
                }

                setImportData(parsed);
                showToast(`${parsed.length} productos detectados correctamente`);
            } catch (error) {
                console.error(error);
                showToast("Error al procesar el archivo. Asegúrate de usar la plantilla.", "error");
            }
        };
                        ...
                        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                                <div
                                    ...
                                    className="p-8 bg-emerald-50 border-2 border-emerald-100 rounded-[2.5rem] cursor-pointer hover:bg-emerald-100 transition-all group"
                                >
                                    <div className="flex items-center gap-4 mb-4">
                                        <div className="p-3 bg-white text-emerald-600 rounded-2xl shadow-sm"><FileSpreadsheet size={24} /></div>
                                        <div className="text-left">
                                            <p className="font-black text-slate-900 uppercase text-xs">Paso 1</p>
                                            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Descargar Plantilla</p>
                                        </div>
                                    </div>
                                    <p className="text-xs text-slate-600 font-medium">Obtén el formato oficial de Excel con las columnas necesarias para una importación sin errores.</p>
                                </div>

                                <div className="relative p-8 bg-sky-50 border-2 border-sky-100 rounded-[2.5rem] hover:bg-sky-100 transition-all group">
                                    <input type="file" accept=".xlsx,.xls,.csv" onChange={handleFileUpload} className="absolute inset-0 opacity-0 cursor-pointer z-10" />
                                    <div className="flex items-center gap-4 mb-4">
                                        <div className="p-3 bg-white text-sky-600 rounded-2xl shadow-sm"><FileUp size={24} /></div>
                                        <div className="text-left">
                                            <p className="font-black text-slate-900 uppercase text-xs">Paso 2</p>
                                            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Subir Archivo</p>
                                        </div>
                                    </div>
                                    <p className="text-xs text-slate-600 font-medium">Selecciona el archivo Excel o CSV completado para procesar los datos del catálogo.</p>
                                </div>
                            </div>

                            <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-[2.5rem] p-12 text-center mb-8 relative">
                                <FileUp size={48} className="mx-auto mb-4 text-slate-300" />
                                <p className="font-bold text-slate-600">Previsualiza tus datos aquí abajo</p>
                                <p className="text-[10px] text-slate-400 uppercase tracking-widest mt-2">Soporta .xlsx, .xls y .csv</p>
                            </div>
