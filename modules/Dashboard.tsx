...
const Dashboard: React.FC = () => {
    ...
    const [dashboardStats, setDashboardStats] = useState({ revenue: 0, activeLeads: 0, todayApts: 0 });
    const [alerts, setAlerts] = useState<any[]>([]);
    const [financialTrend, setFinancialTrend] = useState<any[]>([]);

    const [aiLoading, setAiLoading] = useState(false);
    const [aiBriefing, setAiBriefing] = useState<string | null>(null);
    const [weather, setWeather] = useState({ temp: 24, status: 'Cargando...', city: 'Querétaro, MX' });
    ...
    const fetchData = async () => {
        try {
            ...
            const [statsRes, alertsRes, weatherRes, healthRes, trendRes] = await Promise.all([
                fetch('/api/dashboard/stats', { headers }).then(r => r.ok ? r.json() : null),
                fetch('/api/dashboard/alerts', { headers }).then(r => r.ok ? r.json() : []),
                fetch('/api/weather').then(r => r.ok ? r.json() : null),
                fetch('/api/health').then(r => r.ok ? true : false).catch(() => false),
                fetch('/api/reports/financial?months=6', { headers }).then(r => r.ok ? r.json() : [])
            ]);

            if (statsRes) {
                setDashboardStats(statsRes);
                if (!aiBriefing) generateDailyBriefing(statsRes.activeLeads, 100);
            }

            if (alertsRes) setAlerts(alertsRes);
            if (weatherRes) setWeather(weatherRes);
            if (trendRes) setFinancialTrend(trendRes);
    ...
    useEffect(() => {
        fetchData();
    }, [generateDailyBriefing, aiBriefing]);
    ...
    if (loading) return (
        <div className="h-screen flex flex-col items-center justify-center text-slate-400">
            ...
            <p className="font-black text-xs uppercase tracking-widest">Sincronizando con el Centro de Control...</p>
        </div>
    );
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 pb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                    { label: 'Ingresos Totales', val: formatMXN(dashboardStats.revenue), icon: TrendingUp, color: 'text-emerald-500', bg: 'bg-emerald-50', sub: 'Montos en Pesos MXN' },
                    { label: 'Servicios de Hoy', val: dashboardStats.todayApts, icon: Calendar, color: 'text-sky-500', bg: 'bg-sky-50', sub: 'Instalación y Mant.' },
                    { label: 'Leads en Pipeline', val: dashboardStats.activeLeads, icon: Magnet, color: 'text-indigo-500', bg: 'bg-indigo-50', sub: 'Oportunidades de Venta' },
                        ...
                        {financialTrend.length > 0 ? (
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={financialTrend}>
                                    ...
                                    <Area type="monotone" dataKey="ingresos" stroke="#0ea5e9" strokeWidth={4} fill="url(#gradTrend)" />
                                </AreaChart>
                            </ResponsiveContainer>
                        ) : (
                            <div className="flex flex-col items-center justify-center h-full text-slate-300">
                                <Activity size={48} className="mb-4 opacity-20" />
                                <p className="text-[10px] font-black uppercase tracking-widest">Sin datos históricos suficientes</p>
                            </div>
                        )}
                    </div>
                </div>
                        ...
                        <div className="space-y-6 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                            {alerts.map((alert, i) => (
                                <div key={i} className="flex gap-4 p-5 bg-white/5 rounded-2xl border border-white/10 hover:bg-white/10 transition-colors cursor-pointer group">
                                    <div className={`w-1.5 h-full rounded-full group-hover:scale-y-110 transition-transform ${alert.type === 'Urgent' ? 'bg-rose-500' : 'bg-sky-500'}`} />
                                    <div>
