...
# Easypanel pasará la API_KEY configurada en sus variables de entorno de construcción.
ARG API_KEY
ENV API_KEY=$API_KEY
...
# La variable API_KEY será capturada por vite.config.ts y quemada en el cliente
COPY . .
RUN npm run build
...
# El API_KEY también es necesario en runtime para los servicios de Gemini en el backend
ARG API_KEY
ENV API_KEY=$API_KEY
