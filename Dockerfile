...
# Copia de código fuente y ejecución del build (genera la carpeta /dist)
COPY . .
RUN npm run build
...
# Instalación de dependencias de producción únicamente (ahorra espacio)
COPY --chown=node:node package*.json ./
RUN npm config set fetch-retries 5 && \
    npm install --omit=dev

# Copiar el código del servidor y los archivos estáticos compilados del Stage 1
